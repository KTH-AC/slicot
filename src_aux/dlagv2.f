...
*     November 2006
*     V. Sima, February 2009: additional WI initialization;
*                             better efficiency in some DROT calls.
*
*     .. Scalar Arguments ..
      INTEGER            LDA, LDB
...
*
      ELSE IF( ABS( B( 1, 1 ) ).LE.ULP ) THEN
         ...
         CSR = ONE
         SNR = ZERO
         CALL DROT( 1, A( 1, 2 ), LDA, A( 2, 2 ), LDA, CSL, SNL )
         CALL DROT( 1, B( 1, 2 ), LDB, B( 2, 2 ), LDB, CSL, SNL )
         A( 1, 1 ) = R
         A( 2, 1 ) = ZERO
         B( 1, 1 ) = ZERO
         B( 2, 1 ) = ZERO
...
*
      ELSE IF( ABS( B( 2, 2 ) ).LE.ULP ) THEN
         ...
         SNR = -SNR
         CALL DROT( 1, A( 1, 1 ), 1, A( 1, 2 ), 1, CSR, SNR )
         CALL DROT( 1, B( 1, 1 ), 1, B( 1, 2 ), 1, CSR, SNR )
         CSL = ONE
         SNL = ZERO
         A( 2, 1 ) = ZERO
         A( 2, 2 ) = T
